    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('orders.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out">
                    Back to Orders
                </a>
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-2">Provider Notes</p>
                    ...
                    @if($order->status === 'completed')
                        ...
                        @elseif(auth()->id() === $order->client_id)
                            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                                <div class="p-6">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Leave a Review</h3>
                                    <p class="text-gray-600 mb-4">How was your experience with this service?</p>
                                    <a href="{{ route('reviews.create', $order) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out">
                                        Write Review
                                    </a>
                                </div>
                            </div>
                ...
                <div class="space-y-6">
                    ...
                    @if(auth()->id() === $order->client_id && $order->canBeCancelled())
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Cancel Order</h3>
                            <p class="text-gray-600 mb-4">You can cancel this order if it hasn't been started yet.</p>
                            <form method="POST" action="{{ route('orders.cancel', $order) }}" onsubmit="return confirm('Are you sure you want to cancel this order?')">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out">
                                    Cancel Order
                                </button>
                            </form>
                        </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="space-y-2 text-sm">
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-900">{{ $order->contact_email }}</span>
                                </div>
                                @endif
                            </div>
                        </div>
